...
int get_event_json(TString dstFilePath = "", enumTracking tracking = VectorFinder)
{
    if (dstFilePath == "")
    {
    ...
    for (Int_t iEvent = 2; iEvent < 3; iEvent++)
    //for (Int_t iEvent = 0; iEvent < dataChain->GetEntries(); iEvent++)
    {
        ...
        for (Int_t iTrack = 0; iTrack < stsTracks->GetEntriesFast(); iTrack++) {
            ...
            // Convert one track to JSON - works -- PK
            //TString trackjson = TBufferJSON::ToJSON(track);
            //cout << trackjson;
